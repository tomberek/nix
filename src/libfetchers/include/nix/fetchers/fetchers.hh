...
/**
 * Evidence that a particular `Input` is final and has a NAR hash ---
 * the precondition `substituteFromStore()` and
 * `InputScheme::tryRealizeLocally()` require. The only way to obtain
 * one is `Input::asFinal()`, so a caller holding a `FinalInput`
 * doesn't need to (and cannot forget to) recheck the precondition.
 */
struct FinalInput
{
    const Input & input;
    ...
    /**
     * Realize `input` via a locally-valid store path or substitution,
     * without invoking the scheme's fetcher at all.
     *
...
private:
    FinalInput(const Input & input, Hash narHash)
        : input(input)
...
public:
    ...
    /**
     * If `isFinal() && getNarHash()`, return evidence of that fact.
     * Used to gate operations, like `FinalInput::substituteFromStore()`,
     * that only make sense for such inputs.
     */
    std::optional<FinalInput> asFinal() const;
...
struct InputScheme
{
    ...
    /**
     * Called with evidence that `input.isFinal()` and has a NAR hash,
     * before `getAccessor()`. Gives the scheme a chance to realize
     * `input` more cheaply than a full fetch, e.g. via store
     * substitution or the scheme's own cache.
     *
    ...
    tryRealizeLocally(const FinalInput & input, const Settings & settings, Store & store) const
    {
